...
if (peer.remaining_time !== undefined) {
    ...
    expiryText = "Not Set"; 
            ...
            content.innerHTML = `
                ...
                <p>Group: <span class="peer-group-display">${peer.group || 'N/A'}</span></p>
                <p>Tags: <span class="peer-tags-display">${peer.tags && peer.tags.length > 0 ? peer.tags.join(', ') : 'N/A'}</span></p>
            `;
...
window.applyFilter = applyFilter;
    ...
    const openEditPeerModal = (peer) => {
        ...
        // Populate group and tags fields
        document.getElementById("editPeerGroup").value = peer.group || ""; 
        document.getElementById("editPeerTags").value = peer.tags ? peer.tags.join(", ") : "";
    ...
    const editPeerGroup = document.getElementById("editPeerGroup").value.trim() || null;
    const editPeerTagsString = document.getElementById("editPeerTags").value.trim();
    const editPeerTags = editPeerTagsString ? editPeerTagsString.split(',').map(tag => tag.trim()).filter(tag => tag) : [];
    ...
    const payload = {
        ...
        group: editPeerGroup, // Add group to payload
        tags: editPeerTags    // Add tags to payload
    };
...
function exportPeers() {
    const selectedConfig = document.getElementById('configSelector') ? document.getElementById('configSelector').value : 'wg0.conf';
    if (!selectedConfig) {
        alert('Please select a configuration file first.');
        return;
    }
    // Construct the URL for exporting peers, ensuring the config_name is passed as a query parameter
    const exportUrl = `/api/export-peers?config_name=${encodeURIComponent(selectedConfig)}`;

    // Use a temporary anchor element to trigger the download
    const anchor = document.createElement('a');
    anchor.href = exportUrl;
    // The browser will infer the filename from the Content-Disposition header set by the server
    // or you can set a default if needed, though it's better handled server-side.
    // anchor.download = selectedConfig.replace('.conf', '') + '_peers.json'; // Example: wg0_peers.json
    document.body.appendChild(anchor); // Required for Firefox
    anchor.click();
    document.body.removeChild(anchor);

    showNotification('Peer data export initiated.');
}



function loadAndDisplayPeers(configName, page = 1, limit = 10) {
// ...existing code...
}

    // Event listener for submitting the Add Peer form
    const addPeerForm = document.getElementById("addPeerForm");
    if (addPeerForm) {
        addPeerForm.addEventListener("submit", async (event) => {
            event.preventDefault();
            ...
            const expiryMonths = parseInt(document.getElementById("newPeerExpiryMonths").value) || 0;
            const expiryDays = parseInt(document.getElementById("newPeerExpiryDays").value) || 0;
            const expiryHours = parseInt(document.getElementById("newPeerExpiryHours").value) || 0;
            const expiryMinutes = parseInt(document.getElementById("newPeerExpiryMinutes").value) || 0;
            const group = document.getElementById("newPeerGroup").value.trim() || null; // Get group value, or null if empty
            const tagsString = document.getElementById("newPeerTags").value.trim();
            const tags = tagsString ? tagsString.split(',').map(tag => tag.trim()).filter(tag => tag) : []; // Split and trim tags, ensure no empty tags

            if (!peerName || !peerIp || !limitValue) {
                showAlert("Peer Name, IP, and Data Limit are required.");
                return;
            }
            ...
            const payload = {
                ...
                peerIp,
                limit,
                dns: dns || "1.1.1.1, 1.0.0.1", // Default DNS if empty
                persistentKeepalive: persistentKeepalive ? parseInt(persistentKeepalive) : 25,
                mtu: mtu ? parseInt(mtu) : 1280,
                expiryMonths,
                expiryDays,
                expiryHours,
                expiryMinutes,
                configFile: currentConfig, // Ensure currentConfig is available in this scope
                group, // Add group to payload
                tags   // Add tags to payload
            };
            ...
            try {
                ...
                const data = await response.json();
                if (response.ok) {
                    showAlert(data.message || "Peer created successfully!");
                    document.getElementById("addPeerModal").style.display = "none";
                    addPeerForm.reset();
                    fetchPeers(currentConfig); // Refresh peer list
                } else {
                    showAlert(data.error || "Failed to create peer.");
                }
            } catch (error) {
                console.error("Error creating peer:", error);
                showAlert("An error occurred while creating the peer.");
            }
        });
    }
    ...
    // Open Add Peer Modal button (if you have one, e.g., with id 'addPeerBtn')
    const addPeerBtn = document.getElementById("addPeerBtn");
    if (addPeerBtn) {
        addPeerBtn.addEventListener("click", () => {
            // Potentially pre-fill or reset form fields here if needed
