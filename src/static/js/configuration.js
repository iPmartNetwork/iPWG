...
(function () {
    ...
    const toggleClass = (selector, className, condition) => {
        document.querySelectorAll(selector).forEach(ele => {
            condition ? ele.classList.add(className) : ele.classList.remove(className);
        });
    };
    ...
    /**
     * Show alert on the configuration page
     * @param response
     */
    function configurationAlert(response) {
        const alerts = [];
        if (response.listen_port === "" && response.status === "stopped") {
            alerts.push(createElement("div", ["alert", "alert-warning"], { role: "alert" }, 
    ...
    function configurationHeader(response) {
        ...
        $conf_status_btn.innerHTML = `
            <a href="#" id="${response.name}" ${response.checked} class="switch">
                <label class="wg-switch">
                    <input type="checkbox" ${response.checked ? "checked" : ""}>
                    <span class="wg-slider wg-round"></span>
                </label>
            </a>`;
        $conf_status_btn.classList.remove("info_loading");

        updateElementText("#conf_status", `${response.status}<span class="dot dot-${response.status}"></span>`);
        updateElementText("#conf_connected_peers", response.running_peer);
        updateElementText("#conf_total_data_usage", `${response.total_data_usage[0]} GB`);
        updateElementText("#conf_total_data_received", `${response.total_data_usage[2]} GB`);
        updateElementText("#conf_total_data_sent", `${response.total_data_usage[1]} GB`);
        updateElementText("#conf_public_key", response.public_key);
        updateElementText("#conf_listen_port", response.listen_port || "N/A");
        updateElementText("#conf_address", response.conf_address);

        toggleClass("#sort_by_dropdown option", "selected", false);
        document.querySelector(`#sort_by_dropdown option[value="${response.sort_tag}"]`).setAttribute("selected", "selected");

        toggleClass(".interval-btn-group button", "active", false);
        document.querySelector(`button[data-refresh-interval="${response.dashboard_refresh_interval}"]`).classList.add("active");

        toggleClass(".display-btn-group button", "active", false);
        document.querySelector(`button[data-display-mode="${response.peer_display_mode}"]`).classList.add("active");

        toggleClass(".info h6", "info_loading", false);
    }

    /**
     * Parse and display peers list
     * @param response
     */
    function configurationPeers(response) {
        ...
        if (response.peer_data.length === 0) {
            peerListContainer.innerHTML = `<div class="col-12 text-center mt-4"><h3 class="text-muted">هیچ کاربری وجود ندارد</h3></div>`;
            return;
        }

        const displayMode = response.peer_display_mode === "list" ? "col-12" : "col-sm-6 col-lg-4";
        const peersHTML = response.peer_data.map(peer => {
            ...
            return `
                <div class="${displayMode}" data-id="${peer.id}">
                    <div class="card mb-3 card-${peer.status}">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-sm display d-flex align-items-center mb-2">
                                    <h6 class="text-uppercase m-0 ml-auto">
                                        <span class="dot dot-${peer.status}" data-toggle="tooltip" title="کاربر متصل است!"></span>
                                    </h6>
                                    <h5 class="m-0 ml-2 text-truncate">${peer.name || "Untitled"}</h5>
                                </div>
                                <div class="w-100"></div>
                                <div class="col-12 peer_data_group mb-2 pr-2">
                                    <p class="text-primary text-uppercase mb-0 ml-2 float-left">
                                        <small><i class="bi bi-arrow-down-right"></i> ${roundN(totalReceive, 4)}GB</small>
                                    </p>
                                    <p class="text-success text-uppercase mb-0 float-left ml-3">
                                ...
                                <div class="col-12">
                                    <small class="text-muted d-flex">
                                        ...
                                        <strong class="ml-auto text-primary opacity-0 transition-opacity">جهت کپی کلیک کنید</strong>
                                    </small>
                                    <h6 class="col-12 text-left">
                                        <samp class="ml-auto key public_key_mobile">${peer.id}</samp>
                                    </h6>
                                </div>
                                <div class="col-12">
                                    <small class="text-muted"><strong>آی پی وایرگارد کاربر :</strong></small>
                                    <h6 class="col-12 text-left text-uppercase">${peer.allowed_ip}</h6>
                                </div>
                                <div class="col-12">
                                    <small class="text-muted"><strong>آخرین اتصال :</strong></small>
                                    <h6 class="col-12 text-left text-uppercase">${peer.latest_handshake}</h6>
                                </div>
                                <div class="w-100"></div>
                                <div class="col-12">
                                    <small class="text-muted"><strong>آی پی کاربر :</strong></small>
                                    <h6 class="col-12 text-left text-uppercase">${peer.endpoint}</h6>
                                </div>
                                <div class="col-sm text-right">
                                    <small class="text-muted"><strong>وضعیت :</strong></small>
                                    <h6 class="text-uppercase">${peer.end_active ? 'فعال' : 'غیرفعال'}</h6>
                                </div>
                                <div class="w-100"></div>
                                <div class="col-12">
                                    <hr>
                                    <div class="button-group d-flex">
                                        <button type="button" class="btn btn-outline-primary btn-setting-peer btn-control" id="${peer.id}" data-toggle="modal">
                                            <i class="bi bi-gear-fill" data-toggle="tooltip" title="تنظیمات کاربر"></i>
                                        </button>
                                        <button type="button" class="btn btn-outline-danger btn-delete-peer btn-control" id="${peer.id}" data-toggle="modal">
                                            <i class="bi bi-x-circle-fill" data-toggle="tooltip" title="حذف کاربر"></i>
                                        </button>
                                        ${peer.private_key ? `
                                            <div class="share_peer_btn_group ml-auto d-inline">
                                                <button type="button" class="btn btn-outline-success btn-qrcode-peer btn-control" data-imgsrc="/qrcode/${response.name}?id=${encodeURIComponent(peer.id)}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" style="width: 19px;" fill="#28a745">
                                                ...
                                                <a href="/download/${response.name}?id=${encodeURIComponent(peer.id)}" class="btn btn-outline-info btn-download-peer btn-control">
                                                    <i class="bi bi-download" data-toggle="tooltip" title="دانلود کاربر"></i>
                                                </a>
                                            </div>` : ''}
                                    </div>
                                </div>
    ...
    window.configurations = {
        ...
        deleteBulkModal: () => {
            return modals.deleteBulkModal;
        },
        deleteModal: () => {
            return modals.deleteModal;
        },
        ipModal: () => {
            return modals.ipModal;
        },
        qrcodeModal: () => {
            return modals.qrcodeModal;
        },
        settingModal: () => {
            return modals.settingModal;
        },
