async function loadSettings() {
    ...
    const themeToggleButton = document.getElementById('theme-toggle-button'); // We'll add this button to settings.html
    const currentTheme = localStorage.getItem('theme');

    function setTheme(theme) {
        document.body.classList.remove('light-mode'); // Remove light-mode first
        if (theme === 'light') {
    ...
    if (currentTheme) {
        ...
        if (themeToggleButton && currentTheme === 'light') {
            // Assuming the button text/icon might change based on theme
            // For example, if it shows a sun icon in dark mode and moon in light mode
