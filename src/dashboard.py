"""
< Wireguard Panel > - Copyright(C) 2021 Donald Zou customaized by iPmartNetwork
...
DASHBOARD_CONF = os.path.join(configuration_path, 'iPWGDashboard.ini')
...
def get_all_configs_total_data():
    """
    Get total data usage for all configurations.
    @return: dict with total_sent, total_received, and total_usage
    """
    total_sent = 0
    total_received = 0
    for name in get_config_names():
        data = get_conf_total_data(name)
        total_received += data[1] # upload_total is index 1 in get_conf_total_data
        total_sent += data[2]     # download_total is index 2 in get_conf_total_data
    total_usage = total_sent + total_received
    # Convert to GB for better readability
    return {
        "total_sent": round(total_sent / (1024**3), 2),
        "total_received": round(total_received / (1024**3), 2),
        "total_usage": round(total_usage / (1024**3), 2)
    }
...
def inject_global_data():
    ...
    if getattr(g, 'db', None) is None:
        g.db = connect_db()
        g.cur = g.db.cursor()
    all_configs_data = get_all_configs_total_data()
    return dict(
        all_configs_total_sent=all_configs_data['total_sent'],
        all_configs_total_received=all_configs_data['total_received'],
        all_configs_total_usage=all_configs_data['total_usage'],
        conf_list_for_navbar=get_conf_list()  # To populate sidebar/navbar links
    )
...
def check_update():
    ...
    config = get_dashboard_conf()
    try:
        data = urllib.request.urlopen("https://api.github.com/repos/iPmartNetwork/iPWG/releases").read()
