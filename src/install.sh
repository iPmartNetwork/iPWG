...
help() {
  ...
  display_logo2
  printf "${YELLOW}=================================================================================\n"
  printf "${YELLOW}+     ${BLUE}<iPWG Dashboard> by iPmartNetwork ${BLUE}https://github.com/iPmartNetwork        ${YELLOW}+\n"
  printf "${YELLOW}=================================================================================${NC}\n"
  printf "${YELLOW}| Usage: ${GREEN}./wgd.sh <option>${NC}                                                      ${YELLOW}|\n"
  printf "${YELLOW}|                                                                               ${YELLOW}|\n"
  printf "${YELLOW}| Available options:                                                            ${YELLOW}|\n"
  printf "${YELLOW}|    ${GREEN}start${NC}: To start iPWG Dashboard.                                           ${YELLOW}|\n"
  printf "${YELLOW}|    ${GREEN}stop${NC}: To stop iPWG Dashboard.                                             ${YELLOW}|\n"
  printf "${YELLOW}|    ${GREEN}debug${NC}: To start iPWG Dashboard in debug mode (i.e., run in foreground).   ${YELLOW}|\n"
  printf "${YELLOW}|    ${GREEN}install${NC}: To install iPWG Dashboard                                        ${YELLOW}|\n"
  printf "${YELLOW}=================================================================================${NC}\n"
}
_check_and_set_venv(){
...
RESET='\033[0m'
    ...
    if [ $version_pass == "0" ]; then
        print_box "iPWG Dashboard requires Python 3.7 or above" "${RED}"
        exit 1
    fi
...
gunicorn_start() {
  ...
  print_box "Starting iPWG Dashboard with Gunicorn in the background." "$YELLOW"
...
print_box() {
  ...
  printf "${color}╚${dashes}╝${RESET}\n"
    ...
    printf "%s\n" "$dashes"
    print_box "iPWG Dashboard in the foreground." "${GREEN}"
    python3 "$app_name"
    printf "%s\n" "$dashes"
}
...
update_wgd() {
  ...
  read up
  if [ "$up" = "Y" ]; then
    printf "| Shutting down iPWG Dashboard...                             |\n"
...
if [ "$#" != 1 ]; then
  ...
  if [ "$1" = "start" ]; then
    if check_wgd_status; then
      printf "%s\n" "$dashes"
      printf "${GREEN}| iPWG Dashboard is already running.                          ${NC}|\n"
      printf "%s\n" "$dashes"
    else
      printf "%s\n" "$dashes"
      print_box "Starting iPWG Dashboard with Gunicorn in the background." "$BLUE"
      start_wgd
    fi
  elif [ "$1" = "stop" ]; then
    if check_wgd_status; then
      printf "%s\n" "$dashes"
      stop_wgd
      print_box "iPWG Dashboard is stopped." "$GREEN"
      printf "%s\n" "$dashes"
    else
      printf "%s\n" "$dashes"
      print_box "iPWG Dashboard is stopped." "$GREEN"
      printf "%s\n" "$dashes"
      printf "${GREEN}| iPWG Dashboard is not running.                              ${NC}|\n"
      printf "%s\n" "$dashes"
    fi
  elif [ "$1" = "update" ]; then
    update_wgd
    ...
    if check_wgd_status; then
      ...
      printf "%s\n" "$dashes"
      print_box "iPWG Dashboard is not running. Starting it now." "$GREEN"
      start_wgd
    fi
  elif [ "$1" = "debug" ]; then
    if check_wgd_status; then
