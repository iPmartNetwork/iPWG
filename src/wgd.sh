...
# wgd.sh - Copyright(C) 2021 Donald Zou customaized by iPmartNetwork
...
help () {
  printf "=================================================================================\n"
  printf "+          <iPWGDashboard> by  Donald Zou customaized by iPmartNetwork           +\n"
  printf "=================================================================================\n"
...
install_wgd(){
    ...
    printf "| iPWGDashboard installed successfully!                      |\n"
...
gunicorn_start () {
  printf "%s\n" "$dashes"
  printf "| Starting iPWGDashboard with Gunicorn in the background.    |\n"
  if [ ! -d "log" ]; then
    mkdir "log"
  fi
  d=$(date '+%Y%m%d%H%M%S')
...
start_wgd_debug() {
  printf "%s\n" "$dashes"
  printf "| Starting iPWGDashboard in the foreground.                  |\n"
  python3 "$app_name"
  printf "%s\n" "$dashes"
}
...
update_wgd() {
  ...
  read up
  if [ "$up" = "Y" ]; then
    printf "| Shutting down iPWGDashboard...                             |\n"
...
if [ "$#" != 1 ];
  then
    ...
    if [ "$1" = "start" ]; then
        if check_wgd_status; then
          printf "%s\n" "$dashes"
          printf "| iPWGDashboard is already running.                          |\n"
          printf "%s\n" "$dashes"
          else
            start_wgd
        fi
        ...
        if check_wgd_status; then
            printf "%s\n" "$dashes"
            stop_wgd
            printf "| iPWGDashboard is stopped.                                  |\n"
            printf "%s\n" "$dashes"
            else
              printf "%s\n" "$dashes"
              printf "| iPWGDashboard is not running.                              |\n"
              printf "%s\n" "$dashes"
        fi
      elif [ "$1" = "update" ]; then
        update_wgd
